...
function CountdownClock( targetElement ) {
  ...
  this.template = tmpl(
    '<div class="ttn-countdown-clock" style="background-color: <%= backgroundColor %>  !important; color: <%= fontColor %> !important; font-size: <%= fontSize %>px !important; !important">' +
      ...
      '<div class="embed-clock-copyright" style="margin-top: 10px; background-color: white; padding: 2px; ">' +
        '<a target="_blank" href="<%= linkUrl %>" style="text-decoration: none; text-align: center; display: block !important; font-size: 12px"><%= linkText %></a>' +
      '</div>' +
    '</div>' +
    '<noscript>If the countdown is not showing, Javascript may be disabled. Please visit <a href="https://www.thetimenow.com/">www.thetimenow.com</a> for support.</noscript>'
  );
  ...
  this.updateSettings = function(){
    ...
    console.log( day, hour, minute, second, 0 );

    this.endTime = new Date( day, hour, minute, second, 0 );

    var timeDiffParts = timeDiff( this.endTime );
    
    this.hours   = timeDiffParts[0];
    this.minutes = timeDiffParts[1];
    this.seconds = timeDiffParts[2];
  };
  ...
  this.render = function(){
    this.clock = this.template({
      ...
      linkText:        "&copy; The Time Now",
      linkUrl:         "https://www.thetimenow.com/javascript/clock"
    });
...
function TimerClock( targetElement ) {
  ...
  this.template = tmpl(
    '<div class="ttn-timer-clock" style="background-color: <%= backgroundColor %> !important;  color: <%= fontColor %> !important; font-size: <%= fontSize %>px !important; ">' +
...
function ClockClock( targetElementIdx, hour, minute, second, date, text, url ) {
  ...
  this.template = tmpl(
    '<div class="ttn-thetimenow-clock" style="background-color: <%= backgroundColor %>;  color: <%= fontColor %>; font-size: <%= fontSize %>px; ">' +
      '<span class="ttn-thetimenow-clock-hours"><%= hour %></span>:<span class="ttn-thetimenow-clock-minutes"><%= minute %></span>:<span class="ttn-thetimenow-clock-seconds"><%= second %></span>' +
      '<div class="ttn-thetimenow-clock-date"><%= date %></div>' +
      '<div class="embed-clock-copyright" style="margin-top: 10px; background-color: white; padding: 2px; ">' +
        '<a target="_blank" href="<%= linkUrl %>" style="text-decoration: none; text-align: center; display: block !important; font-size: 12px"><%= linkText %></a>' +
      '</div>' +
    '</div>'
...
function doJsonpRequest( params, callback ) {
  ...
  // construct URL
  url = 'https://www.thetimenow.com/ttn-embed-js.php?' + queryString + '&callback=' + callback;
...
(function() {
  ...
  styleTag.setAttribute( "href", 'https://www.thetimenow.com/min/css/ttn-embed.css' );
